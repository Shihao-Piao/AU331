...
def test_LabelPropagation(*data):
    ...
    #cls=LabelPropagation(max_iter=10000,kernel='rbf',gamma=100,tol=0.01)
    cls = LabelPropagation(max_iter=1000, n_neighbors=3,  kernel='knn')
...
def test_LabelSpreading(*data):
    ...
    Y_train=np.copy(Y)
    Y_train[unlabeled_index]=-1
    cls=LabelSpreading(max_iter=100,kernel='rbf',gamma=2)
    cls.fit(X,Y_train.ravel())
